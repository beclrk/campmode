...
interface MapViewProps {
  ...
  crownIds?: Set<string>;
  /** When true, zoom and layer controls are hidden (e.g. when location sheet is open). */
...
function MapZoomControls({ hideWhenSheetOpen }: { hideWhenSheetOpen?: boolean }) {
  const map = useMap();
  return (
    <div className={`absolute bottom-[calc(var(--app-promo-banner-height,72px)+8rem+env(safe-area-inset-bottom,0px))] right-4 md:bottom-[calc(var(--app-promo-banner-height,72px)+1rem+env(safe-area-inset-bottom,0px))] z-[1100] flex flex-col gap-1 rounded-xl overflow-hidden shadow-lg border border-neutral-700 bg-neutral-900/95 backdrop-blur-sm ${hideWhenSheetOpen ? 'hidden' : ''}`}>
      <button
        type="button"
        onClick={() => map.zoomIn()}
...
function MapLayerControl({
  ...
  return (
    <div ref={ref} className={`absolute bottom-[calc(var(--app-promo-banner-height,72px)+8rem+env(safe-area-inset-bottom,0px))] left-4 md:bottom-[calc(var(--app-promo-banner-height,72px)+1rem+env(safe-area-inset-bottom,0px))] z-[1100] ${hideWhenSheetOpen ? 'hidden' : ''}`}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
...
export default function MapView({
  ...
  return (
    ...
    >
      ...
      <MapZoomControls hideWhenSheetOpen={locationSheetOpen} />
      <MapLayerControl basemap={basemap} onBasemapChange={onBasemapChange} osApiKey={osApiKey} hideWhenSheetOpen={locationSheetOpen} />
