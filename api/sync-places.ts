...
/** Fetch UK EV chargers from OpenChargeMap. Uses countrycode=GB; API key required (OPENCHARGEMAP_API_KEY in Vercel). */
async function fetchAllOcmUK(): Promise<SupabaseLocationRow[]> {
  const ocmKey = process.env.OPENCHARGEMAP_API_KEY;
  const base = `${OCM_BASE}?output=json&countrycode=GB&maxresults=10000&compact=true&verbose=false`;
  const url = ocmKey ? `${base}&key=${encodeURIComponent(ocmKey)}` : base;
  const urlForLog = ocmKey ? `${base}&key=[REDACTED]` : base;
  try {
    const resp = await fetch(url);
    console.log('[sync-places] OCM API response status:', resp.status, resp.statusText, 'url:', urlForLog);
    if (!resp.ok) {
